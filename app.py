        ...
        if request.method == "POST":
            if "file" in request.files:
                ...
                if df is not None:
                    ...
                    if x_axis and y_axis and plot_type:
                        ...
                        if plot_type == "scatter":
                            ...
                            sns.violinplot(data = df[x_axis])
                        elif plot_type == "swarm":
                            sns.swarmplot(data = df[x_axis])
                        buf = io.BytesIO()
                ...
                if is_classification:
                    ...
                    buf.close()
                    plt.close()
            elif request.form.get("action") == "Download Report":
                try:
                    # Create a comprehensive text report
                    report_content = "Model Performance Report\n"
                    report_content += "=" * 30 + "\n\n"
                    ...
                    if mse is not None:
                        report_content += f"Mean Squared Error: {mse}\n"
                    
                    if mae is not None:
                        report_content += f"Mean Absolute Error: {mae}\n"
                    
                    if auc_score is not None:
                        report_content += f"AUC Score: {auc_score}\n\n"

                    # Add detailed classification/regression report
                    if report:
                        report_content += "Detailed Performance Report:\n"
                        report_content += "-" * 30 + "\n"
                    ...
                    if preprocess_message:
                        ...
                        report_content += "-" * 30 + "\n"
                        report_content += preprocess_message + "\n"

                    # Add model parameters
                    report_content += "\nModel Parameters:\n"
                    report_content += "-" * 30 + "\n"
                    report_content += f"Learning Rate: {learning_rate}\n"
                    report_content += f"Number of Estimators: {n_estimators}\n"
                    report_content += f"Max Depth: {max_depth}\n"
                    report_content += f"Optimization Method: {optimization_method}\n"

                    # Create a BytesIO object to store the file content
                    from io import BytesIO
                    report_file = BytesIO(report_content.encode())
                    report_file.seek(0)

                    # Send the file directly from BytesIO
                    return send_file(
                        report_file, 
                        mimetype='text/plain', 
                        as_attachment=True, 
                        download_name='model_performance_report.txt'
                    )
